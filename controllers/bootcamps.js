...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if(publishedBootcamp && req.user.role !== 'admin'){
    return next(new errorResponse(`The user with ID ${req.user.id} has already published one bootcamp`, 404))
  }
...
exports.getBootcampsInRadius = asyncHandler(async (req, res, next) => {
  ...
  // Distance/Radius of Earth(3,633 mi)
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  if(file.size > process.env.MAX_FILE_UPLOAD){
    return next(new errorResponse(`Please upload a image < than ${process.env.MAX_FILE_UPLOAD}`, 400));
  }
